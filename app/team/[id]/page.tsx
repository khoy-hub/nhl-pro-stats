...
export default async function TeamPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    // ЗАПРОС 1: Получаем статистику (Очки, Голы)
    const statsData = await getClubStats(id);

    // ЗАПРОС 2: Получаем состав (Номера джерси)
    const rosterData = await getTeamRoster(id);

    // === МАГИЯ СЛИЯНИЯ ===
    // Создаем "Словарь номеров": { id_игрока: номер }
    const numberMap = new Map<number, number>();

    // Пробегаем по всем игрокам в ростере и запоминаем их номера
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                    {goaliesWithNumbers.map((player) => (
                        <div key={player.playerId} className="relative group bg-slate-900 border border-slate-800 rounded-2xl p-6 hover:border-blue-500/50 transition-all overflow-hidden">
                            {/* Номер (теперь точно есть) */}
                            ...
                            <div className="relative z-10 flex items-center gap-4 mt-2">
                                ...
                                <div>
                                    <h3 className="font-bold text-lg">{player.firstName.default} {player.lastName.default}</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {sortedSkaters.map((player) => (
                        <div
                            key={player.playerId}
                            className="relative group overflow-hidden bg-slate-900 border border-slate-800 rounded-2xl p-5 hover:bg-slate-800/80 hover:border-slate-600 transition-all duration-300"
                        >
                            {/* Номер справа снизу (теперь точно есть) */}
                            ...
                            <div className="relative z-10 mt-6 pt-4 border-t border-slate-800 flex justify-between text-sm font-mono text-slate-400">
                                <div className="flex flex-col items-center">
                                    ...
                                    <span className="text-[10px] uppercase">Goals</span>
                                </div>
                                <div className="flex flex-col items-center">
                                    <span className="text-white font-bold text-lg">{player.assists}</span>
                                    <span className="text-[10px] uppercase">Assists</span>
                                </div>
                                <div className="flex flex-col items-center">
                                    <span className="text-white font-bold text-lg">{player.gamesPlayed}</span>
                                    <span className="text-[10px] uppercase">Games</span>
                                </div>
                                <div className="flex flex-col items-center">
